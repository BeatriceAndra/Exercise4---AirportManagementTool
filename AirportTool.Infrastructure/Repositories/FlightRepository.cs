...
namespace AirportTool.Infrastructure.Repositories
{
    public class FlightRepository : Repository<Flight>, IFlightRepository
    {
        ...
        public FlightRepository(AirportManagementContext context, IMapper mapper)
            : base(context, mapper)
        {
            _context = context;
            _mapper = mapper;
        ...
        public async Task<Flight?> GetFlightWithSchedulesAsync(
            int flightId,
            CancellationToken cancellationToken = default)
        {
            var flightEf = await _context.Flights
                .Include(f => f.FlightSchedules)
                .FirstOrDefaultAsync(f => f.Id == flightId, cancellationToken);

            return flightEf == null
                ? null
                : _mapper.Map<Flight>(flightEf);
        }

        public async Task<IEnumerable<Flight>> GetFlightsByRouteAsync(
            int originAirportId,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            {
                query = query.Where(f =>
                    f.FlightSchedules.Any(fs =>
                        fs.ScheduledDepartureUtc.Date == date.Value.Date));
            }
